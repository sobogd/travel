...
// A→B connection search (direct / via-hub / via-nearby-airport) on a date,
// from the earliest departure time, with custom radius + max layover.
export async function POST(req: NextRequest) {
  try {
    ...
    const tStart = String(body.tStart || "").trim(); // "YYYY-MM-DDTHH:MM" local at A
    const maxDistKm = Math.max(0, Math.min(1000, Number(body.maxDistKm) || 0));
    const maxLayoverMin = Math.max(60, Math.min(2880, Number(body.maxLayoverMin) || 240));
    ...
    if (!TS.test(tStart)) return NextResponse.json({ error: "bad time" }, { status: 400 });

    let itineraries;
    try {
      itineraries = await connectSearch({ origin, dest, tStartLocal: tStart, maxDistKm, maxLayoverMin });
    } catch (e) {
